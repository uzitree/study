...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "36ad364ab617de78824c";
...
/******/ 	return hotCreateRequire("./src/vrplayer.js")(__webpack_require__.s = "./src/vrplayer.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(/*! ./../_webpack@4.39.3@webpack/buildin/global.js */ "./node_modules/_webpack@4.39.3@webpack/buildin/global.js"), __webpack_require__(/*! ./../_webpack@4.39.3@webpack/buildin/module.js */ "./node_modules/_webpack@4.39.3@webpack/buildin/module.js")(module)))
...
/***/ "./node_modules/_webpack@4.39.3@webpack/buildin/module.js":
/*!***********************************!*\
...
/*!*************************!*\
  !*** ./src/vrplayer.js ***!
  \*************************/
/*! exports provided: default, VrPlayer */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return VrPlayer; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "VrPlayer", function() { return VrPlayer; });
...
// Load the full build.
var _ = __webpack_require__(/*! lodash */ "./node_modules/_lodash@4.17.15@lodash/lodash.js");

var VrPlayer =
/*#__PURE__*/
function () {
  function VrPlayer(el, options) {
  ...
  _createClass(VrPlayer, [{
    ...
    value: function checkSetting() {
      ...
      if (!this.el) {
        ...
        if (typeof this.settings.videoPath === 'string') {
          ...
          vPath = this.settings.videoPath.bq;
        }
    ...
    value: function setVideoDuration() {
      var sec = this.playerObj.api_getVideoDom().duration;
      this.duration = sec;
    ...
    key: "customPlayer",
    value: function customPlayer() {
      var _this = this;

      var liveMode = this.settings.videoType === 'm3u8';
      /*
      ...
      */
      ...
      this.el.addEventListener('click', function (event) {
        ...
        setTimeout(() => {
          if (this.playerObj.api_getVideoPlayStatus()) {
            if (this.customControlBar.target.style.display === 'none') {
              if (this.customControlBar) {
      ...
      this.customControlBar = customControlBar(this.el, {
        doPlay_click: function doPlay_click(status) {
          if (status === 'press') {
            _this.playerObj.api_changerSceneById('sv');

            _this.playerObj.api_setVideoPlay();
          } else {
            _this.playerObj.api_setVideoPause();
          }

          console.log('doPlay_click', status);
        },
        doMore_click: function doMore_click(status) {
          console.log('doMore_click', status);
        },
        doFullscreen_click: function doFullscreen_click(status) {
          ...
          if (fullscreenEnabled) {
            ...
            if (status === 'press') {
              // 全屏
              _this.playerObj.api_enterFullScreen();
            } else {
              _this.playerObj.api_exitFullScreen();
            }
          } else {
            if (status === 'press') {
              // 全屏
              _this.el.classList.add('fullscreen');
            } else {
              _this.el.classList.remove('fullscreen');
            }
          }
        },
        doQt_change: function doQt_change(status) {
          // 切换码率
          if (typeof _this.settings.videoPath !== 'string' && _this.settings.videoPath[status]) {
            var curTime = _this.playerObj.api_getVideoCurTime(); // const playStatus = this.playerObj.api_getVideoPlayStatus()


            var vPath = _this.settings.videoPath[status];

            _this.playerObj.api_changeVideoPath(vPath);

            _this.playerObj.api_setVideoCurTime(curTime);
          }
      ...
      this.customPlayButton = customPlayButton(this.el, {
        ...
        playButton_click: function playButton_click(status) {
          if (status === 'press') {
            _this.playerObj.api_changerSceneById('sv');

            _this.playerObj.api_setVideoPlay();

            if (_this.customControlBar) {
              _this.customControlBar.show();
            }

            if (_this.customProgressBar) {
              _this.customProgressBar.show();
            }
      ...
      if (!liveMode) {
        ...
        this.customProgressBar = customProgressBar(this.el, {
          seekTo: function seekTo(x, width) {
            // 视频seek到指定到位置
            _this.playerObj.api_setVideoCurTime(x / width * _this.duration);
          }
        });
    ...
    value: function initialize() {
      ...
      // 检查配置
      var utovrSettings = this.checkSetting();
      if (!utovrSettings) return; // 加载中界面

      var startTm = new Date().getTime();
      var preDom = document.createElement('div');
      this.el.appendChild(preDom);
      preDom.id = this.el.id + '_pre';
      preDom.className = 'vrplayer_pre'; // 播放器回调处理

      utovrSettings.videoPlayerCallBack = function () {
        console.log('playing');

        _this2.customPlayButton.switchToPauseIcon();

        _this2.customControlBar.switchToPauseIcon();
      };

      utovrSettings.videoTogglePlayCallBack = function () {
        console.log('pause');

        _this2.customPlayButton.switchToPlayIcon();

        _this2.customControlBar.switchToPlayIcon();
      };

      utovrSettings.videoPlayEndCallBack = function () {
        console.log('end');
      ...
      utovrSettings.sceneEventClickCallBack = function () {
      ...
      utovrSettings.videoLoadProgressCallBack = function (e) {
        if (_this2.customProgressBar) {
          var videoDom = _this2.playerObj.api_getVideoDom();

          if (videoDom) {
            var buf = videoDom.buffered;
            var leg = buf.length - 1;
            if (leg < 0) return;
            var t = buf.end(leg).toFixed(1);

            _this2.customProgressBar.loadProgress(t);
          }
        }
      };

      utovrSettings.videoUpdateCallBack = function (e) {
        var t = _this2.playerObj.api_getVideoCurTime();

        if (_this2.customControlBar) {
          _this2.customControlBar.setCurrentTime(t);
        }

        if (_this2.customProgressBar) {
          _this2.customProgressBar.playProgress(t);
        }
      };

      utovrSettings.videoLoadMetaDataCallBack = function (e) {
        console.log('videoLoadMetaDataCallBack', e); // 视频已加载，记录视频时长

        _this2.setVideoDuration();
      };

      loadPlayer(utovrSettings).then(function (playerObj) {
        _this2.playerObj = playerObj;

        _this2.customPlayer(); // 添加自定义播放器样式


        playerObj.api_changerSceneByIndex(0);
        playerObj.api_littlePlanet();
        setTimeout(function () {
          preDom.style.display = 'none'; // 隐藏loading动画

          _this2.playerObj.api_normalView();
        }, 1000); // 延时一秒优化小行星效果

        if (typeof _this2.settings.ready === 'function') {
          _this2.settings.ready();
        }

        var endTm = new Date().getTime();
        var usedTm = endTm - startTm;
...
function loadPlayer(utovrSettings) {
  return new Promise(function (resolve, reject) {
    ...
    if (typeof window.initLoad === 'function') {
      window.initLoad(utovrSettings); // console.log(utovrSettings)
    }
  });
}

function customPlayButton(el, options) {
  var btnDom = options.playButton;
...
function customProgressBar(el, options) {
  ...
  var playedDom = document.createElement('div');
  ...
  o.useDoms.bg = bgDom; // 加载进度

  o.loadProgress = function (t) {
    var self = this;
    if (!self.duration) return;
    var w = (t / self.duration * 100).toFixed(1) + '%';
...
function customControlBar(el, options) {
  ...
  barDom.appendChild(rightDom); // 播放、暂停

  leftDom.innerHTML += '<div id="' + barDom.id + '_doPlay" class="vrplayer_cb_doPlay"></div>'; // 时间

  leftDom.innerHTML += '<div id="' + barDom.id + '_time" class="vrplayer_cb_time">' + '<span class="current">00:00</span><span class="sep">/</span><span class="duration">00:00</span></div>'; // 清晰度

  rightDom.innerHTML += '<div id="' + barDom.id + '_doQt" class="vrplayer_cb_doQt"><span>标清</span><div>' + '<h3 class="bq">标清</h3><h3 class="gq">高清</h3><h3 class="cq">超清</h3></div></div>'; // 全屏

  rightDom.innerHTML += '<div id="' + barDom.id + '_doFullscreen" class="vrplayer_cb_doFullscreen"></div>'; // 更多

  rightDom.innerHTML += '<div id="' + barDom.id + '_doMore" class="vrplayer_cb_doMore"></div>'; // 点击事件处理

  document.querySelectorAll('#' + barDom.id + ' > div > div').forEach(function (item) {
    item.addEventListener('click', function (event) {
      event.stopPropagation();
      if (this.classList.contains('vrplayer_cb_time')) return; // 点击时间不处理

      var eName = this.id.replace(barDom.id + '_', '') + '_click';
      var status = 'press';
  ...
  document.querySelectorAll('#' + barDom.id + '_doQt h3').forEach(function (item) {
    item.addEventListener('click', function (event) {
  ...
  o.useDoms.duration = document.querySelector('#' + barDom.id + ' .duration'); // 动态设置工具状态
  ...
  o.switchQtIconStatus = function (select) {// const self = this
  ...
  o.setCurrentTime = function (sec) {
    var self = this;
    var dt = new Date(self.zeroTime + sec * 1000);

    if (sec >= 60 * 60) {
      // >= 1 Hour
      self.useDoms.current.innerText = _.padStart(dt.getHours(), 2, '0') + ':' + _.padStart(dt.getMinutes(), 2, '0') + ':' + _.padStart(dt.getSeconds(), 2, '0');
    } else {
  ...
  o.setDuration = function (sec) {
    ...
    if (sec >= 60 * 60) {
      // >= 1 Hour
      self.useDoms.duration.innerText = _.padStart(dt.getHours(), 2, '0') + ':' + _.padStart(dt.getMinutes(), 2, '0') + ':' + _.padStart(dt.getSeconds(), 2, '0');
    } else {
...
/******/ })));
//# sourceMappingURL=vrplayer.win.js.map